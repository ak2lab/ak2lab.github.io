...
export default class {
  render({ cases }) {
    ...
    <section class="frame works" id="works"><div class="section-heading"><h2>制作事例</h2><p>目的、使用技術、制作で考えたことを、事例ごとのリポジトリにまとめています。</p></div><div class="case-grid">${cases.map(item => `<article class="case"><a href="${e(item.repository)}">${item.image ? `<div class="case-image"><img src="/${e(item.image)}" alt="${e(item.title)}の画面" loading="lazy" width="1440" height="900"></div>` : ''}<p class="category">${e(item.category)}</p><h3>${e(item.title)} <span aria-hidden="true">↗</span></h3></a><p>${e(item.summary)}</p></article>`).join('')}</div></section>`);
  }
}
